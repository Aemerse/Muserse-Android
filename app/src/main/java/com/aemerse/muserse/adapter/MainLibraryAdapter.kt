...
import com.aemerse.muserse.activity.ActivityTagEditor
import com.aemerse.muserse.activity.FragmentLibrary
...
class MainLibraryAdapter(fl: FragmentLibrary?, private val context: Context, data: ArrayList<dataItem>) : RecyclerView.Adapter<MainLibraryAdapter.MyViewHolder?>(), PopupMenu.OnMenuItemClickListener, FastScrollRecyclerView.SectionedAdapter, BubbleTextGetter {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        when (fl!!.getStatus()) {
            Constants.FRAGMENT_STATUS.TITLE_FRAGMENT -> {
                ...
                if (!ApplicationClass.getPref().getBoolean(context.getString(R.string.pref_data_saver), false)) builder = Glide
                    .with(context)
                    .load(url)
                    .centerCrop()
            ...
            Constants.FRAGMENT_STATUS.ALBUM_FRAGMENT -> {
                ...
                val trackCoun: String = when {
                        trackCount > 1 -> trackCount.toString() + context.getString(R.string.tracks)
                        else -> trackCount.toString() + context.getString(R.string.track)
                    }
    ...
    fun sort(sort_id: Int) {
        val sort_order: Int = ApplicationClass.getPref()
        ...
        when (fl!!.getStatus()) {
            Constants.FRAGMENT_STATUS.TITLE_FRAGMENT -> try {
                ...
                files.add(FileProvider.getUriForFile(context,
                    context.applicationContext.packageName + "com.aemerse.music.provider",
                    fileToBeShared))
